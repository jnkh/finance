...
%JUST FOR THIS ROUND--THE P OF HAVING 1 IS TOO HIGH
net{n}{numhid,k,numdelay}.divideFcn='divideint'; 
end
...
outputtrdiff{n}=cell2mat(output{n}(tr{n}.testInd))-cell2mat(output{n}((tr{n}.testInd)-1)); 
numuptog=sum((targetstrdiff{n}>0).*(outputtrdiff{n}>0)); 
numdowntog=sum((targetstrdiff{n}<0).*(outputtrdiff{n}<0));
numtog=numuptog+numdowntog; 
resultstrtog{n}(numhid,k,numdelay)=numtog/numel(tr{n}.testInd); 
...
differ=targetmat{n}(tr{n}.testInd)-cell2mat(output{n}(tr{n}.testInd)); 
%ups only
...
%divide by total number of testing values
resultstrdiff{n}(numhid,k,numdelay)=resultstrdiff{n}(numhid,k,numdelay)/numel(tr{n}.testInd); 
...
for n=1:numnet; 
    for k=1:numdelay; 
     resultstog{n}(:,:,k)=resultstog{n}(:,:,k)/(300-period-(k*stepsized+mind-stepsized)-1); 
     resultsdiff{n}(:,:,k)=resultsdiff{n}(:,:,k)/(300-period-(k*stepsized+mind-stepsized)-1); 
    end
end
...
r17iresults=iresults; 


save('r17iresults','r17iresults')
...
r17avresults=avresults; 
save('r17avresults','r17avresults')
